...
namespace dtaalbers.Ionic.Api.Controllers
{
    ...
    public class MediaController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Post(IFormFile media)
        {
            if (media == null) return BadRequest("Unable to find media");
            ...
            var orignal = Path.Combine(_apiSettings.UploadsDirectory, media.FileName);
            ...
            var contentType = $"image/{Path.GetExtension(media.FileName).Replace(".", "")}";
